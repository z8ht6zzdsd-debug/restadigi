      ...
      {plan.tables.map((table) => {
        ...
        return (
          <button
            ...
            title={`Pöytä ${table.label} · ${table.seats} hlö · ${ZONE_LABELS[table.zone]}`}
          >
...
function DashboardFloorPlanPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!res.ok) throw new Error("Pöytäkartan lataus epäonnistui");
  ...
  async function resetDemo() {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Demokartan palautus epäonnistui");
      const data = (await res.json()) as { plan: FloorPlan; stats: Stats };
      setPlan(data.plan);
      setStats(data.stats);
      setSelectedId(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <p className="text-muted-foreground">Ladataan pöytäkarttaa…</p>
      ) : (
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard label="Asiakaspaikat" value={`${stats?.seats ?? "—"} / ${plan.capacity}`} />
            <StatCard label="Pöytiä" value={String(stats?.tableCount ?? plan.tables.length)} />
            <StatCard
              label="2 / 4 hlö"
            ...
            <aside className="space-y-4">
              <div className="rounded-sm border border-border bg-card p-4">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Klikkaa pöytää nähdäksesi tiedot. Tämä on hallintapaneelin malli — tuotannossa
                  kartta rakennetaan ravintolan pohjapiirroksen ja paikkaluvun mukaan.
                </p>
              </div>
              ...
              {selected ? (
                <div className="rounded-sm border border-border bg-card p-4">
                  ...
                  <p className="mt-1 text-lg font-medium">Pöytä {selected.label}</p>
                  <dl className="mt-3 space-y-2 text-sm">
                    <div className="flex justify-between gap-2">
                      <dt className="text-muted-foreground">Paikkoja</dt>
                      <dd className="font-medium">{selected.seats} hlö</dd>
                    </div>
                    <div className="flex justify-between gap-2">
                      <dt className="text-muted-foreground">Alue</dt>
                      <dd className="font-medium">{ZONE_LABELS[selected.zone]}</dd>
                    </div>
                    <div className="flex justify-between gap-2">
                      <dt className="text-muted-foreground">Muoto</dt>
                      <dd className="font-medium">
                        {selected.shape === "round" ? "Pyöreä" : "Suorakaide"}
                      </dd>
                    </div>
                  </dl>
                </div>
              ...
              <div className="rounded-sm border border-border bg-card p-4">
                <h4 className="text-sm font-medium">Kustomointi ravintolalle</h4>
                <p className="mt-2 text-sm text-muted-foreground">
                  Esimerkki sekoituksesta {plan.capacity} paikalle: {suggested[2]} × 2 hlö,{" "}
                  {suggested[4]} × 4 hlö, {suggested[6]} × 6 hlö, {suggested[8]} × 8 hlö.
                </p>
                <ul className="mt-3 space-y-1.5 text-sm text-foreground/80">
                  <li>· Pohja asiakaspaikkaluvusta (esim. 40 / 80 / 120)</li>
                  <li>· Pöytäkoot 2–8 hlö ravintolan tyylin mukaan</li>
                  <li>· Alueet: ikkuna, sali, terassi, kabinetti</li>
                </ul>
              </div>
            </aside>
          </div>
          ...
          <section className="space-y-3">
            <h3 className="font-medium">Kaikki pöydät</h3>
            <div className="overflow-x-auto rounded-sm border border-border">
              <table className="w-full min-w-[28rem] text-left text-sm">
                <thead className="border-b border-border bg-secondary/40 text-xs uppercase tracking-wide text-muted-foreground">
                  <tr>
                    <th className="px-3 py-2 font-medium">Pöytä</th>
                    <th className="px-3 py-2 font-medium">Paikat</th>
                    <th className="px-3 py-2 font-medium">Alue</th>
                    <th className="px-3 py-2 font-medium">Muoto</th>
                  </tr>
                </thead>
                <tbody>
                  {plan.tables.map((t) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 tabular-nums">{t.seats} hlö</td>
                      <td className="px-3 py-2">{ZONE_LABELS[t.zone]}</td>
                      <td className="px-3 py-2">{t.shape === "round" ? "Pyöreä" : "Suorakaide"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
